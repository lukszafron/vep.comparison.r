...
f.vep.comparison <- function(Robject1, Robject2) {
        ...
        if(length(all.genes) > 0 & length(sig.genes) > 0) {
          ...
          if (length(de) > 0) {
            ...
            if(exists('p1')) {if(nrow(p1$data) > 0) {assign(p1.name, value = p1)
              p1.pathways <- as.character(p1$data[order(p1$data$GeneRatio, decreasing = T), , drop = F][["Description"]][1:if(nrow(p1$data)<3){nrow(p1$data)} else {3}])
              if(index == 1) {
  ...
  load(Robject1)
  load(Robject2)
  workdir <- file.path(arguments2[1], "COMPARISON")
...
impacts <- all.impacts[str_contains(x = grep(ls(), pattern = "^anno\\.SNP", value = T), pattern = all.impacts)]
  ...
  for(impact in impacts) {
    ...
    res1.bool.snp <- res1.bool.snp[order(rownames(res1.bool.snp)), , drop = F]
    res1.bool.non.snp <- get(df.non.snp.name)
    res1.bool.non.snp <- res1.bool.non.snp[order(rownames(res1.bool.non.snp)), , drop = F]
    ...
    snp.non.snp.df <- snp.non.snp.df[order(snp.non.snp.df[[Indfactor]]), , drop = F]
...
workdir <- file.path(arguments2[1], "COMPARISON")
runid <- sub(workdir, pattern = "(.*\\/RUNS\\/)(.*)(\\/(MAPPINGS|MAPPINGS_TRIMMED)\\/.*)", replacement = "\\2")
...
Robject1.dir <- file.path(workdir, "..", "SNP")
Robject2.dir <- file.path(workdir, "..", "NON_SNP")
